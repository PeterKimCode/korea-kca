...
관리자 화면에서 입력한 모든 해시태그는 전체강좌 페이지의 필터로 자동 표시됩니다. `돌봄`, `상담`, `디지털`, `안전`, `문화`가 먼저 나오고 나머지 태그는 가나다순으로 정렬됩니다. 같은 태그를 여러 번 입력해도 하나만 저장됩니다.

저장소 기본 콘텐츠를 직접 수정하는 경우에는 `courseTagMap`의 키와 `courseGroups`의 강좌명이 띄어쓰기까지 같아야 합니다.
...
관리자 화면에서 강좌가 `공개 중`인지 확인합니다. 저장소 기본 콘텐츠를 직접 수정했다면 `courseTagMap`의 강좌명이 `courseGroups`의 강좌명과 정확히 같은지도 확인합니다.
