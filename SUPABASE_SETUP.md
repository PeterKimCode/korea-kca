...
이 문서는 개발 담당자가 **처음 한 번만** 진행합니다. 연결이 끝나면 운영자는 GitHub나 코드를 열지 않고 `홈페이지주소/admin/`에서 강좌, 공지, 슬라이드, 교재를 관리합니다.
...
현재 저장소의 강좌, 공지, 슬라이드 중 아직 등록되지 않은 항목만 Supabase로 복사됩니다. 교재 초기 데이터는 없으며 관리자에서 직접 등록합니다. 관리자가 직접 만든 콘텐츠와 기존 수정 내용은 그대로 유지되며, 같은 기본 데이터는 중복 등록되지 않습니다.
